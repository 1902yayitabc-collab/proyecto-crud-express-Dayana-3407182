const manejandoErroresMiddleware = (error,req, res, next) => {
   const codigoError = error.statusCode || 500
   const mensajeError = error.message || "Error inesperado"
   console.error(`{Errror} ${new Date().toISOString()} - ${req.method}, ${req.url}, ${req.ip}`)
...
module.exports = manejandoErroresMiddleware